...
public class StudentDAO {
	...
	public List<Student> selectAll() {
		String query = "SLELCT * FROM STUDENT_TBL";
	...
	public List<Student> selectAllByName(String studentName) {
		String query = "SLELCT * FROM STUDENT_TBL WHERE STUDENT_NAME = ?";
		List<Student> sList = new ArrayList<Student>();
		Connection conn = null;
...
//			ResultSet rset = stmt.executeQuery(query);
	...
	public Student selectOneById(String studentId) {
		// 1. 위치홀더 셋팅
		// 2. PreparedStatement 객체 생성 with qurty
		// 3.입력값 셋팅
		// 4. 쿼리문 실행 및 결과 받기(feat. method())
		String query = "SLELCT * FROM STUDENT_TBL WHERE STUDENT_ID = ?";
...
//			result = stmt.executeUpdate(query);
	...
	public int updateStudent(Student student) {
		String query = "UPDATE STUDNET_TBL SET STUDENT_PWD = ?, EMAIL =?, PHONE =?, ADDRESS = ?, HOBBY =? WHERE STUDENT_ID = ?";
		int result = -1;
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
...
//			result = stmt.executeUpdate(query);
	...
	public Student selectLoginInfo(Student student) {
		String query = "SLELCT * FROM STUDENT_TBL WHERE STUDENT_ID = ? AND STUDENT_PWD = ?";
		Student result = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		try {
			...
			pstmt = conn.prepareStatement(query);
			pstmt.setString(1, student.getStudentId());
			pstmt.setString(2, student.getStudentPwd()); // 시작은 1로하고 마지막 수는 물음표의 갯수와 같다.(물음표 = 위치홀더)
			rset = pstmt.executeQuery();
//			Statement stmt = conn.createStatement();
//			ResultSet rset = stmt.executeQuery(query);
			if (rset.next()) {
